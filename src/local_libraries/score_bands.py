...
def plot_confusion_matrix(df):
    labels = ['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'Run-off', 'uwr_rejection']#sorted(df.recommended_category.unique().tolist())#
    migration_matrix = confusion_matrix(df.score_category_declared, df.recommended_category, labels=labels, normalize='true')
    ...
    plt.savefig(os.path.join("figures", "impact_analysis", 'migration_matrix.png'), dpi=600, bbox_inches='tight')
    plt.show()
    return


def plot_confusion_matrix2(df, normalization, filename):
    labels = ['PricingIneligibility', 'EquifaxIneligibility', 'uwr_rejection', 'Pre-accepted', 'inconsistent']#sorted(df.recommended_category.unique().tolist())#
    ...
    plt.tight_layout()
    os.makedirs(os.path.join("figures", "impact_analysis"), exist_ok=True)
    plt.savefig(os.path.join("figures", "impact_analysis", "pre_acceptance_"+filename+".png"), dpi=600, bbox_inches='tight')
    plt.show()
    return